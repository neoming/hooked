...
import com.main.hooker.hooker.adapter.FollowerAdapter;
...
public class FollowerActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        adapter = new FollowerAdapter(R.layout.item_follow_user, mFollowers);
        initRefresh();
        initLoadMore();
    ...
    private void initNav() {
        ...
        if(isUserSelf()){
            navTitle.setText("Followers");
            addBtn.setVisibility(View.VISIBLE);
        }else{
            navTitle.setText(mUser.username + "'s Followers");
    ...
    private void load(boolean more, Runnable callback) {
        ...
        new Thread(()->{
            try {
                ArrayList<Follow> list = UserModel.getFollowedBy(mUser.id, mPage);
                if(list == null)
                    throw new ApiFailException();
                ArrayList<User> userList = new ArrayList<>();
                for (Follow follow : list){
                    if(follow.from_user != null)
                        userList.add(follow.from_user);
                }
                runOnUiThread(()->{
                    if(mPage == 1){
